...
const EducationCard = ({ Icon, title, subtitle, degree, listItems }) => {
  ...
  return (
    <animated.div style={loadProps}>
      <animated.div
        style={hoverProps}
        onMouseEnter={() => setHover({ transform: 'translate3d(0,-10px,0)', boxShadow: '0px 12px 18px -6px rgba(0, 0, 0, 0.5)' })}
        onMouseLeave={() => setHover({ transform: 'translate3d(0,0px,0)', boxShadow: '0px 6px 10px -4px rgba(0, 0, 0, 0.1)' })}
      >
